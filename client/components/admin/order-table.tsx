...
import { CheckCircle2, Clock, MoreHorizontal, Package, Truck } from "lucide-react";

const ORDERS = [
  { id: "ORD-902102", store: "TechHaven", customer: "John Doe", date: "Oct 24, 2026", total: 323.99, status: "Delivered" },
  { id: "ORD-902099", store: "Minimalist Desk Co", customer: "Alice Smith", date: "Oct 22, 2026", total: 145.50, status: "Processing" },
  { id: "ORD-901844", store: "TechHaven", customer: "Robert Chen", date: "Oct 20, 2026", total: 899.00, status: "Shipped" },
  { id: "ORD-901523", store: "Fashion Forward", customer: "Emily Johnson", date: "Oct 15, 2026", total: 49.99, status: "Cancelled" },
];
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900/50 overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-900/50">
        <h2 className="font-semibold text-slate-900 dark:text-white text-lg">Global Order Feed</h2>
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
            {ORDERS.map((order) => (
              <tr key={order.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/20 transition-colors">
                <td className="px-6 py-4 font-medium text-slate-900 dark:text-white font-mono text-xs">{order.id}</td>
                <td className="px-6 py-4 text-indigo-600 dark:text-indigo-400 font-medium">{order.store}</td>
