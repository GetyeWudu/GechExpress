...
class Inventory(SoftDeleteModel):
    ...
    class Meta:
        ...
        constraints = [
            ...
            models.UniqueConstraint(
                ...
                condition=Q(is_deleted=False, product__isnull=False, product_variant__isnull=True),
    ...
    def __str__(self):
        target = self.product_variant.sku if self.product_variant_id else (
            self.product.sku if self.product_id else "unassigned"
        )
    ...
    def clean(self):
        if not self.product_id and not self.product_variant_id:
            raise ValidationError("Inventory requires a product or product variant.")
        if self.quantity < 0:
    ...
    def reserve_for_variant(cls, *, product_variant, quantity):
        ...
        total_available = sum(inventory.available_quantity for inventory in inventories)
