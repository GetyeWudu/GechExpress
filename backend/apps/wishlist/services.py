...
class WishlistService:
    ...
    @staticmethod
    @transaction.atomic
    def add_to_wishlist(*, user, product_id=None, product_variant_id=None):
        wishlist = WishlistService.get_or_create_wishlist(user)

        if product_id is None and product_variant_id is not None:
            ...
            if not variant:
                raise ValidationError(
                    {"product_variant": "Product variant not found."})
        ...
        if existing:
            raise ValidationError(
                {"wishlist_item": "Item already in wishlist."})
    ...
    def remove_from_wishlist(*, user, wishlist_item_id):
        try:
            ...
            raise ValidationError(
                {"wishlist_item": "Wishlist item not found."})
